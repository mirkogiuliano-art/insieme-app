...
function toRaw(row: CategoryRow): RawLinkCategory {
  return { id: row.id, name: row.name, color: row.color };
}

/** La categoria "Generale" viene già creata insieme al gruppo (funzione
 * create_group), quindi qui non serve più alcun seeding lato client. */
export async function listCategories(groupId: string): Promise<RawLinkCategory[]> {
...
export async function createCategory(groupId: string, name: string, color: string): Promise<RawLinkCategory> {
  const { data, error } = await supabase
    .from('link_categories')
...
export async function renameCategory(id: string, name: string): Promise<void> {
  const { error } = await supabase.from('link_categories').update({ name }).eq('id', id);
  if (error) throw error;
}
...
export async function deleteCategory(id: string): Promise<void> {
  const { error } = await supabase.rpc('delete_link_category', { p_category_id: id });
  if (error) throw error;
}
...
export function subscribeToCategories(
  groupId: string,
  handlers: {
    onInsert: (category: RawLinkCategory) => void;
    onUpdate: (category: RawLinkCategory) => void;
    onDelete: (id: string) => void;
  },
): () => void {
  const uniqueSuffix = Math.random().toString(36).slice(2, 8);
  const channel = supabase
    .channel(`link_categories:${groupId}:${uniqueSuffix}`)
    .on(
      'postgres_changes',
      { event: 'INSERT', schema: 'public', table: 'link_categories', filter: `group_id=eq.${groupId}` },
      (payload) => handlers.onInsert(toRaw(payload.new as CategoryRow)),
    )
    .on(
      'postgres_changes',
      { event: 'UPDATE', schema: 'public', table: 'link_categories', filter: `group_id=eq.${groupId}` },
      (payload) => handlers.onUpdate(toRaw(payload.new as CategoryRow)),
    )
    .on(
      'postgres_changes',
      { event: 'DELETE', schema: 'public', table: 'link_categories', filter: `group_id=eq.${groupId}` },
